    ...
    N: for<'a> FormatFields<'a> + 'static,
{
    fn format_event(&self, ctx: &FmtContext<'_, S, N>, mut writer: Writer<'_>, event: &Event<'_>) -> std::fmt::Result {
        ...
        let mut v = PrettyVisitor::new(f.by_ref(), true, false).with_style(style);
        event.record(&mut v);
        v.finish()?;
        writeln!(f)?;
...
impl<'a> PrettyVisitor<'a> {
    ...
    ///   that writer.
    fn new(writer: Writer<'a>, is_empty: bool, align: bool) -> Self {
        Self {
            //
            writer,
            is_empty,
    ...
    fn record_debug(&mut self, field: &Field, value: &dyn fmt::Debug) {
        ...
        match field.name() {
            ...
            name if name.starts_with("r#") => {
                self.alignment = self.alignment.max(name.len() - 2);
                self.fields.push((name[2..].to_owned(), format!("{:?}", value)));
            ...
            name => {
                self.alignment = self.alignment.max(name.len());
                self.fields.push((name.to_owned(), format!("{:?}", value)));
    ...
    fn write_str(&mut self, s: &str) -> fmt::Result {
        ...
        for ch in s.chars() {
            match ch {
                ...
                ch if ch as u32 >= 0x80 && ch as u32 <= 0x9f => write!(self.inner, "\\u{{{:x}}}", ch as u32)?,
        ...
        for (name, value) in &self.fields {
            {
                let value = &format_args!(
                    ...
                    "",
                    value,
                    align = if self.align { self.alignment - name.len() } else { 0 },
                );
                let padding = if self.is_empty {
                    self.is_empty = false;
                    "\n    "
...
impl FormattedFields {
    pub fn new() -> Self {
        Self { is_empty: true, fields: String::new() }
    }
}

impl<'writer> FormatFields<'writer> for FormattedFields {
    fn format_fields<R: RecordFields>(&self, writer: Writer<'writer>, fields: R) -> fmt::Result {
        let mut visitor = PrettyVisitor::new(writer, self.is_empty, true);
        fields.record(&mut visitor);
        visitor.finish()
    }
}

impl Display for FormattedFields {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
